...
def main():
    ...
    LANGUAGES = {
        "pt": {
            ...
            "classify_info": "Envie um arquivo `.csv` com dados de uma curva de luz e deixe nossa IA fazer a análise.",
        ...
        "en": {
            ...
            "classify_info": "Upload a `.csv` file with light curve data and let our AI do the analysis.",
    ...
    with st.sidebar:
        ...
        visible_options = [t("nav_about"), t("nav_classify"), t("nav_performance")]
        st.session_state.page = st.radio(t("sidebar_nav"), visible_options, key="navigation_radio")
    ...
    # Sobre o Projeto
    if st.session_state.page == t("nav_about"):
    ...
    # Classificar Candidato
    elif st.session_state.page == t("nav_classify"):
        ...
        if "dados" not in st.session_state:
            st.session_state.dados = {}  # Para armazenar os inputs temporários
        if "dados_salvos" not in st.session_state:
            st.session_state.dados_salvos = []  # Para armazenar os dados quando gerar previsão

        def gerar_previsao():
            # Cria um dicionário com os dados atuais dos inputs
            registro = {campo: st.session_state.dados.get(campo, "") for campo in campos}
            # Adiciona à lista de dados salvos
        ...
        with col1:
            # Cria um container cinza
            container = st.container()
            # Usamos st.markdown apenas para aplicar cor de fundo e padding via CSS
            container.markdown(
                """
                <div style='
                    ...
                    height: 100%;
                '>
                    <span style='font-size: 18px; color: #333;'>Cadastro manual</span>
                </div>
                """,
        ...
        with col2:
            st.button("Clique Aqui", on_click=mostrar_campos)
        ...
        if st.session_state.show_fields:
            ...
            if todos_preenchidos:
                st.button("Gerar Previsão", on_click=gerar_previsao, type="primary")
            else:
                st.warning("⚠️ Preencha todos os campos para gerar a previsão.")


    # Performance do Modelo
    elif st.session_state.page == t("nav_performance"):
        ...
        with tab2:
            if os.path.exists('matriz_confusao.png'):
                st.image('matriz_confusao.png', caption=t("matrix_caption"), use_container_width=True)
            else:
